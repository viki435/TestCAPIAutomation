...
from utils import service_instance_utils

from pyVmomi import vim, vmodl
import argparse

def deploy_vm(vc_ip, vc_username, vc_password, ova_path, vc_dc_name, vc_ds_name):

    parser  = argparse.ArgumentParser()
    parser.add_argument("-a", "--host", default = vc_ip)
    parser.add_argument("-b", "--port", default = 443)
    parser.add_argument("-c", "--user", default = vc_username)
    parser.add_argument("-d", "--password", default = vc_password)
    parser.add_argument("-e", "--ova_path", default = ova_path)
    parser.add_argument("-f", "--datacenter_name", default = vc_dc_name)
    parser.add_argument("-g", "--datastore_name", default = vc_ds_name )
    parser.add_argument("-i", "--disable_ssl_verification",  default = True )
    parser.add_argument("-j", "--resource_pool",  default = False )
    args, unknown = parser.parse_known_args()

    print("*" * 30)
    print (args)    
    print (unknown)        
    print (args.datacenter_name)
    print (args.datastore_name)
    print (args.resource_pool)
    print("*" * 30)   

    si = service_instance_utils.connect(args)
